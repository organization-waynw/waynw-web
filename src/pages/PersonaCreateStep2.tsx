...
function LoadingScreen({
  ...
  // 핵심 변경: QUESTIONS 기반 + imagePath (import된 실제 모듈 값)
  const selectedImages = QUESTIONS.map((q) => {
    ...
    return opt.imagePath; // string이 아니라 import된 실제 asset
...
interface CardTrayProps {
  answers: Record<number, { optionIndex: number; customText: string }>;
  totalSteps: number;
  newlyAddedStep: number | null;
}

function CardTray({ answers, totalSteps, newlyAddedStep }: CardTrayProps) {
  return (
    <div className="flex items-center gap-2">
      {Array.from({ length: totalSteps }, (_, i) => {
        const questionId = i + 1;
        const imgPath = getSelectedImagePath(answers, questionId);
        const isNew = newlyAddedStep === questionId;
        ...
        return (
          <div
            ...
            style={{
              ...
              flexShrink: 0,
              animation: isNew
                ? "cardSlideUp 0.45s cubic-bezier(0.22,1,0.36,1)"
                : "none",
            }}
          >
            {imgPath ? (
...
function PersonaCreateStep2() {
  ...
  const [newlyAddedStep, setNewlyAddedStep] = useState<number | null>(null);
  ...
  const handleNext = () => {
    // 트레이 카드 슬라이드업 애니메이션
    const answeredStep = question.id;
    setNewlyAddedStep(answeredStep);
    setTimeout(() => setNewlyAddedStep(null), 500);
  ...
  const handleSubmit = () => {
    ...
    // 현재 answers 상태를 즉시 복사 (렌더 타이밍 고정)
